...
def _refresh_missing_account_profiles():
    ...
    profiles = MacroEngine.get_roblox_profiles(
        account.get("name") for account in missing
    )
    changed = False
                ...
                if linked_owner:
                    ...
                    try:
                        ...
                        try:
                            ...
                            pass
                        return
                    config.set_account_token_status(acc_id, tname, True, info.get("id"))
...
def get_calibration_text(scope):
    ...
    if scope == "fishing":
        ...
        region_keys = fishing_presets.region_keys()
        label = "fishing"
...
def _public_calibration_report(report):
    return {key: value for key, value in report.items() if key != "normalized"}
